...
namespace ExcelFunctions
{
    ...
    public enum Declination { Nominative/*Кто? Что?*/, Genitive/*Кого? Чего?*/, Dative/*Кому? Чему?*/, Accusative/*Кого? Что?*/, Instrumental/*Кем? Чем?*/, Prepositional/*О ком? О чём?*/ };

    public static class RuDateAndMoneyConverter
    {
        static Dictionary<Declination, string[]> monthNames = new Dictionary<Declination, string[]>
    {
        { Declination.Nominative, new []{"", "январь", "февраль", "март", "апрель", "май", "июнь", "июль", "август", "сентябрь", "октябрь", "ноябрь", "декабрь"} },
        { Declination.Genitive, new []{"", "января", "февраля", "марта", "апреля", "мая", "июня", "июля", "августа", "сентября", "октября", "ноября", "декабря"} }
    };
        ...
        static string firstMaleDative = "одному";
        static string firstMaleAccusative = "одному";

        static string firstFemale = "одна";
        static string firstFemaleAccusative = "одну";
        static string firstFemaleDative = "одной";      //одну?
        static string firstFemaleGenitive = "одной";    //?
        ...
        static string secondMaleDative = "двум";
        static string secondFemaleDative = "двум";
        ...
        {
        "", "ста", "двумстам", "тремстам", "четыремстам",
        ...
        //same as thousands
        static string[] thousandsGenitive =
        {
        "", "тысячи", "тысяч", "тысяч"
    };
        ...
        static string[] millionsAccusative =
        {
        "", "миллиона", "миллиона", "миллионов"
    };
        static string[] millionsDative =
        {
        "", "миллиону", "миллионам", "миллионам"
    };
        static string[] millionsGenitive =
        {
        ...
        static string[] billionsAccusative =
        {
        "", "миллиарда", "миллиарда", "миллиардов"
    };
        static string[] billionsDative =
        {
        "", "миллиарду", "миллиардам", "миллиардам"
    };
        static string[] billionsGenitive =
        {
        ...
        static string[] centsGenitive =
        {
        "", "цента", "цента", "центов"
    };
        static string[] centsDative =
        {
        "", "центу", "центам", "центам"
    };
        ...
        {
        "", "доллара США", "долларов США", "долларов США"
    };
        ...
        /// <returns></returns>
        static string DateToTextLongYear(DateTime _date, string _year)
        {
            return String.Format("«{0}» {1} {2}",
                                    _date.Day.ToString("D2"),
                                    MonthName(_date.Month, Declination.Genitive),
        ...
        /// <param name="_date"></param>
        /// <returns></returns>
        static string DateToTextLong(DateTime _date)
        {
            return String.Format("«{0}» {1} {2}",
                                    _date.Day.ToString("D2"),
                                    MonthName(_date.Month, Declination.Genitive),
        ...
        /// <param name="_date"></param>
        /// <returns></returns>
        static string DateToTextQuarter(DateTime _date)
        {
        ...
        /// <param name="_date"></param>
        /// <returns></returns>
        static string DateToTextSimple(DateTime _date)
        {
            return String.Format("{0:dd.MM.yyyy}", _date);
        }
        static int DateQuarter(DateTime _date)
        {
        ...
        private static void AppendTail(int lastRublesDigit, Declination textCase, long copecksAmount, int lastCopecksDigit, bool _useUsd, ref string s)
        {
            string[] wholes = (textCase == Declination.Nominative ? (_useUsd ? dollars:rubles) : 
                (textCase == Declination.Genitive ? (_useUsd ? dollarsGenitive : rublesGenitive) : (_useUsd ? dollarsDative : rublesDative)));
            ...
            string[] cops = _useUsd ? 
                (textCase == Declination.Nominative ? cents : (textCase == Declination.Genitive ? centsGenitive : centsDative)) :
                (textCase == Declination.Nominative ? copecks : (textCase == Declination.Genitive ? copecksGenitive : copecksDative));
        ...
        /// <returns></returns>
        public static string CurrencyToTxt(double _amount, Declination textCase, bool _firstCapital, bool _useUsd)
        {
            //Десять тысяч рублей 67 копеек
        ...
        /// <param name="_firstCapital"></param>
        /// <returns></returns>
        static string CurrencyToTxtFull(double _amount, Declination textCase, bool _firstCapital, bool _useUsd)
        {
            //10 000 (Десять тысяч) рублей 67 копеек
            long rublesAmount = (long)Math.Floor(_amount);
            long copecksAmount = ((long)Math.Round(_amount * 100)) % 100;
            ...
            string s = String.Format("{0:N0} ({1}) ", rublesAmount, NumeralsToTxt(rublesAmount, Declination.Nominative, true, _firstCapital));
        ...
        /// <param name="_firstCapital"></param>
        /// <returns></returns>
        static string CurrencyToTxtShort(double _amount, Declination textCase, bool _firstCapital, bool _useUsd)
        {
            //10 000 рублей 67 копеек        
            long rublesAmount = (long)Math.Floor(_amount);
            long copecksAmount = ((long)Math.Round(_amount * 100)) % 100;
        ...
        /// <param name="_firstCapital"></param>
        /// <returns></returns>
        static string NumeralsToTxt(long _sourceNumber, Declination _case, bool _isMale, bool _firstCapital)
        {
            string s = "";
            ...
            while (number > 0)
            {
                ...
                {
                    case 12:
                        //TODO make if someone needs that..
                    ...
                    case 9:
                        switch (_case)
                        {
                            case Declination.Accusative:
                                s = MakeText(remainder, hundreds, tens, from3till19, secondMale, firstMaleAccusative, billionsAccusative) + s;
                                break;
                            case Declination.Dative:
                                s = MakeText(remainder, hundredsDative, tensDative, from3till19Dative, secondMaleDative, firstMaleDative, billionsDative) + s;
                                break;
                            case Declination.Genitive:
                                s = MakeText(remainder, hundredsGenetive, tensGenetive, from3till19Genetive, secondMaleGenetive, firstMaleGenetive, billionsGenitive) + s;
                                break;
                            default:
                    ...
                    case 6:
                        ...
                        {
                            case Declination.Accusative:
                                s = MakeText(remainder, hundreds, tens, from3till19, secondMale, firstMaleAccusative, millionsAccusative) + s;
                                break;
                            case Declination.Dative:
                                s = MakeText(remainder, hundredsDative, tensDative, from3till19Dative, secondMaleDative, firstMaleDative, millionsDative) + s;
                                break;
                            case Declination.Genitive:
                                s = MakeText(remainder, hundredsGenetive, tensGenetive, from3till19Genetive, secondMaleGenetive, firstMaleGenetive, millionsGenitive) + s;
                                break;
                            default:
                    ...
                    case 3:
                        switch (_case)
                        {
                            case Declination.Accusative:
                                ...
                                break;
                            case Declination.Dative:
                                s = MakeText(remainder, hundredsDative, tensDative, from3till19Dative, secondFemaleDative, firstFemaleDative, thousandsDative) + s;
                                break;
                            case Declination.Genitive:
                                s = MakeText(remainder, hundredsGenetive, tensGenetive, from3till19Genetive, secondFemaleGenetive, firstFemaleGenitive, thousandsGenitive) + s;
                                break;
                            default:
        ...
        static string NumeralsDoubleToTxt(double _sourceNumber, int _decimal, Declination _case, bool _firstCapital)
        {
        ...
        /// <param name="_case"></param>
        /// <returns></returns>
        static string MonthName(int _month, Declination _case)
        {
            string s = "";
        ...
        static string NumeralsRoman(int _number)
        {
            string s = "";
